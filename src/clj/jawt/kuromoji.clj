...
;; thanks, https://gist.github.com/fasiha/d1e521c84b093e9756455d82f6482a38
(def keywordize-pos
  {"代名詞" :pronoun
   "副詞" :adverb
   "助動詞" :auxiliary-verb
...
(def int->pos (into [] (vals keywordize-pos)))
(def pos->int (->> int->pos
                   (map-indexed (fn [i pos] [pos i]))
                   (into {})))
...
(defn token-to-map [token]
  {:lemma (.getLemma token)
   ...
   :part-of-speech (->> [(.getPartOfSpeechLevel1 token)
                        ...
                        (remove #(= % "*"))
                        (mapv #(or (keywordize-pos %)
                                   :unknown-pos)))
   :conjugation (mapv #(or (keywordize-inflection %)
                           :unknown-inflection)
                      (split-dashes (.getConjugationForm token)))
   :conjugation-type (mapv #(or (keywordize-inflection-type %)
